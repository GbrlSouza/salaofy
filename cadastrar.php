...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nome) || empty($cpf) || empty($celular) || empty($cep) || empty($senha)) {
        ...
        if (!$condominio) {
            ...
            // O perfil padrão para novos cadastros é 3 (Morador)
            $id_perfil_morador = 3; 

            $id_novo_usuario = $usuarioManager->registrarUsuario(
                $cpf, $senha, $nome, $celular, $id_perfil_morador
            );

            if ($id_novo_usuario) {
                // 5. Vinculação ao Condomínio
                ...
                if ($vinculo_sucesso) {
                    ...
                    // Limpa os campos para não exibir os dados após o sucesso
                    $_POST = [];
                } else {
                    $mensagem_erro = 'Cadastro realizado, mas falha na vinculação ao condomínio. Contate a administração.';
                }
            } else {
                $mensagem_erro = 'Erro desconhecido ao registrar o usuário no banco de dados.';
            }
        }
    }
}

$pageTitle = "Solicitar Cadastro | Salaofy"; 
    ...
    <div class="card shadow-lg p-4" style="max-width: 500px; width: 100%;">
        <div class="card-body">
            <div class="text-center mb-4">
                <h1 class="text-primary fw-bold">Novo Cadastro</h1>
                <p class="text-muted">Preencha seus dados para solicitar acesso como Morador.</p>
            </div>
